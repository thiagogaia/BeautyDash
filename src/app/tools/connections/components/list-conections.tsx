...
export function ListConections({ data, setEditData, setCurrentData, setCurrentTab }: Props) {
      ...
      <TableBody>
        {data.map((item) => (
          <TableRow key={item.id}>
            ...
            <TableCell className="w-1/6 text-center hidden xsm:table-cell font-medium">
              <span
                className={cn(
                  ...
                  item.status === "ativo" && "border-green-900 bg-green-300 text-green-950"
                )}
              >
                {item.status}
              </span>
            ...
            <TableCell className="w-1/6 text-center hidden sm:table-cell">{item.section}</TableCell>
